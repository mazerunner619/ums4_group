...
public class Teacher implements ActionListener {
    ...
    JScrollPane jsp;
    JTextField jtf;
    String columns[] = { "Name", "DOB", "Phone", "Email", "Course", "ID", "City", "Pincode", "Address" };

    Teacher() throws Exception {
        FRAME = new JFrame();
        ...
        App app = new App();
        int count = 0;
        ResultSet rs = app.s.executeQuery("select count(*) from Teacher");
        if (rs.next()) {
            count = rs.getInt(1);
        }
        ...
        int i = 0;
        rs = app.s.executeQuery("select * from Teacher");
        while (rs.next()) {
        ...
        JMenuItem byRoll = new JMenuItem("ID");
    ...
    public void actionPerformed(ActionEvent action) {
        ...
        if (command == "Home") {
            ...
            try {
                App app = new App();
                int count = 0;
                String query = "select * from Teacher where " + searchby + " '" + searchFor + "'";
                ResultSet rs = app.s
                        .executeQuery("select count(*) from Teacher where " + searchby + " '" + searchFor + "'");
                if (rs.next()) {
                    count = rs.getInt(1);
                }
                System.out.println("count " + count);
